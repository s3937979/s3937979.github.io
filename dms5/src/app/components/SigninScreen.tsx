...
interface SigninScreenProps {
  onGetStarted: (email: string, password: string) => string;
...
export default function SigninScreen({ onGetStarted, onCreateAccount }: SigninScreenProps) {
  ...
  const handleGetStarted = () => {
    const error = onGetStarted(email, password);
  ...
  return (
    <div className="h-full flex flex-col bg-[#EDEDEE] px-4 overflow-hidden">
      ...
      <div className="flex-shrink-0">
        ...
        {errorMessage && (
          <p className="mt-1 text-[#C0392B]" style={{ fontSize: '14px', fontWeight: 700 }}>
            {errorMessage}
          </p>
        )}
      </div>
      ...
      <div className="flex-shrink-0 mt-3 pb-4">
        ...
        <button
          ...
          className="w-full py-2.5 bg-[#2C2C2E] text-white rounded-xl mb-2 transition-transform duration-200 ease-out hover:scale-[1.04] disabled:opacity-50"
